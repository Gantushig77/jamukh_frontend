...
const getRealtor = (id) => {
  return ax_instance.get(`/admin/list-of-admin?sort=ASC&page=${id}&limit=12&role=realtor`);
};
const getliked = () => {
...
const getlike = (id) => {
  return ax_instance.post(`/ads/like-ad?ad_id=${id}`, 
   {'ad_id':id}
...
const getremovelike = (id) => {
  return ax_instance.post(`/ads/remove-liked-ad?ad_id=${id}`, 
   {'ad_id':id}
...
const getSearch = (text ,id ,subCategory ) => {
  return ax_instance.get(`/ads/search-ads?search_query=${text}&page=1&limit=10&category=${id}&order=DESC&subCategory=${subCategory}` 
   );
};

export { getads, getDetail, getCategoryAds, getBlogList, getDetailNews ,getRealtor,getliked,getlike,getremovelike,getSearch};
