...
import { GET_SINGLE_GOODS } from '../../graphql/gql/category/category';
...
export default function GoodsDetail() {
  ...
  const { data, loading } = useQuery(GET_SINGLE_GOODS, {
    fetchPolicy: 'cache-and-network',
    variables: { _id: id },
    onCompleted(data) {
      console.log(data);
      data?.getSingleGoods?.categoryImg &&
        setTheImg(data?.getSingleGoods?.categoryImg?.path);
      let someImgs = [...data?.getSingleGoods?.goodsImg];
      someImgs.unshift(data?.getSingleGoods?.categoryImg);
  ...
  const handleAddBasket = () => {
    if (parseInt(meatAmount) < 1) {
      ...
      history.push('/login');
    } else if (data?.getSingleGoods?.total < 1) {
      handleSnackOpen({
        state: true,
      ...
      createUserOrder({
        variables: {
          ...
          goodsName: data?.getSingleGoods?.parentCategory?.name,
          goodsType: data?.getSingleGoods?.name,
          initDate: Date.now(),
          price: meatAmount * data?.getSingleGoods?.price,
        },
      });
    }
  };
      ...
      <Container disableGutters className={classes.body}>
        ...
        {loading ? (
          ...
          <div className={classes.sizeFitter}>
            <Container className={classes.theContainer}>
              ...
              <Container className={classes.infoContainer}>
                <h2 className={classes.title}>
                  {data?.getSingleGoods?.parentCategory?.name +
                    ' ' +
                    data?.getSingleGoods?.name}
                </h2>
                <p className={classes.description}>{data?.getSingleGoods?.description}</p>
                <div className={classes.statsContainer}>
                  <div className={classes.priceContainer}>
                    <p className={classes.priceLabel}>{`${
                      data?.getSingleGoods?.isUnit ? 'Ширхэг' : 'Кг'
                    } тутмын үнэ : `}</p>
                    <p className={classes.price}>{`${data?.getSingleGoods?.price} ₮`}</p>
                  </div>
                  <div className={classes.priceContainer}>
                    <p className={classes.priceLabel}>Нийт нөөц : </p>
                    <p className={classes.price}>{`${data?.getSingleGoods?.total} ${
                      data?.getSingleGoods?.isUnit ? ' ш' : ' кг'
                    }`}</p>
                  </div>
                </div>
              </Container>
            ...
            {/* Similar Goods */}
            <Section2
              parentId={data?.getSingleGoods?.parentCategory?._id}
