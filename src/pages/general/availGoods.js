...
export default function AvailableGoods() {
    ...
    GET_ACTIVE_PARENT_CATEGORIES,
    {
      ...
      onCompleted(data) {
        getChildren({
          variables: {
            active: true,
            parentId: data?.getActiveParents[0]?._id,
          },
        });
        console.log(data);
      },
  ...
  const handleCategoryChange = useCallback(
    (id) => {
      localStorage.setItem('availGoodsCategory', id);
  ...
  useEffect(() => {
    console.log(id);
    let localId = localStorage.getItem('availGoodsCategory');
    if (id) {
      handleCategoryChange(id);
    } else if (localId !== null && localId.length > 0) {
      handleCategoryChange(localId);
    } else if (
      parentLoading === false &&
      parentCategories?.getActiveParents?.length > 0
    ) {
      setParentCategory(parentCategories?.getActiveParents[0]?._id);
    }
  }, [parentLoading, parentCategories, id, handleCategoryChange]);
  ...
  return (
    <>
      ...
      <Container className={classes.body}>
        <PageAbout
          title={'Бараа'}
          desc={`Манайд одоогоор 100 гаруй нэр төрлийн мах махан бүтээгдэхүүн 
            байгаа бөгөөд та хүссэн бараагаа сонгоод гэртээ хүргүүлэн аваарай. 
            Сонгосон барааныхаа дээр дарж ороод дэлгэрэнгүй мэдээлэлтэй нь танилцана уу.`}
          marginBottom='0'
            ...
            <Container maxWidth={false} disableGutters className={classes.parentList}>
              <List component='nav' aria-label='secondary mailbox folders'>
                {parentLoading ? (
                  ...
                  parentCategories?.getActiveParents?.length > 0 &&
                  parentCategories?.getActiveParents.map((item, index) => (
            ...
            <div style={{ marginBottom: 30 }}>
              {parentCategories?.getActiveParents?.length > 0 && (
                <CustomDropdown
                  ...
                  index={
                    parentCategory
                      ? parentCategories?.getActiveParents?.indexOf(
                          parentCategories.getActiveParents?.filter(
          ...
          <div className={classes.childrenGoodsContainer}>
            <Container maxWidth={false} className={classes.cardContainer}>
              {childLoading ? (
                ...
                childCategories?.childCategories?.categories?.length > 0 &&
                childCategories?.childCategories?.categories?.map((item, index) => (
                  <CardItem
                    onSelect={() => history.push(`/goods-detail?id=${item?._id}`)}
