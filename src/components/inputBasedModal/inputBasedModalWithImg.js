...
export default function InputBasedModal({
  title = '[Гарчиг]',
  ...
  const onImgChange = ({ target: { validity, files } }) => {
    if (validity.valid) {
      console.log(files);
      setImgData(files[0]);
    }
      ...
      <div className={classes.root} style={{ height: height, width: width }}>
        ...
        <div className={`${classes.buttons}`}>
          ...
          <h1
            ...
            onClick={() => {
              let send = data;
              let add = send.push(imgData);
              if (onSubmit) onSubmit(add);
            }}
          >
            {submitButtonTitle}
          </h1>
